...
const Miscellaneous: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        const query = `*[_type == "lyrics" && category->name == "Miscellaneous"] { _id, title, slug }`;
        const fetchedSongs = await client.fetch(query);
        setSongs(fetchedSongs);
      } catch (error) {
        console.error("Error fetching Miscellaneous songs:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-background p-2 font-urdu text-foreground">
      ...
      <h1 className="heading-1">کلام کی فہرست</h1>
      {filtered.length > 0 ? (
        ...
        <p className="text-center mt-8">کوئی کلام موجود نہیں ہے۔</p>
      )}
    </div>
  );
};
